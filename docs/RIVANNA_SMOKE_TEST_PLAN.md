...
Docker Hub (mstaylor/cylon-armada-python)
        │  apptainer pull
        ▼
  Rivanna scratch (cylon-armada.sif)
        │  apptainer exec / run
        ▼
...
| `target/rivanna/scripts/Makefile` | `smoke-cpu`, `smoke-gpu`, `expb-smoke`, `image-pull`, `image-pull-gpu`, `image-pull-uccucx` targets |
...
docker tag $ECR:cylon-armada-fmi-python    qad5gv/cylon-armada-python:latest
...
> **Space warning**: CPU SIF ~6 GB, GPU SIF ~20 GB, UCC/UCX SIF similar to CPU
> (~6-7 GB, slightly larger for the UCX/UCC build). Apptainer's layer cache adds
> 2–3× overhead during the pull. This exceeds the default `$HOME`
...
# UCC/UCX image — Experiment B collective benchmark
...
| `cylon-armada-uccucx-python` image not yet built/pushed to Docker Hub | Pending | Built from `Dockerfile.uccucx.python` (adds UCX v1.19.1 + UCC v1.6.0 from source); push then `make image-pull-uccucx` before `make expb-smoke` can run |
